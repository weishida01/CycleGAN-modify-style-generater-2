...
#         kernels = F.interpolate(input=kernels, size=(256*9, 64, 64), mode='nearest')
        ...
        conv8 = self.conv8(torch.cat([conv2, F.interpolate(conv7, scale_factor=2, mode=self.upMode,align_corners=True)], dim=1))  # 128*128*128
        conv9 = self.conv9(torch.cat([conv1, F.interpolate(conv8, scale_factor=2, mode=self.upMode,align_corners=True)], dim=1))  # 64*256*256
